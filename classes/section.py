...
class Section:
    def __init__(self,
        ...
        if circle:
            ...
            # Cut trailing edge
            idx_TE_bot, idx_TE_top = trailingEdgeThickness(trailing_edge_thickness,
                                                        splines[0])
...
if __name__ == '__main__':
    ...
    # curve_0 = LineString(sta.xy)
    # curves_offset = {0:sta.xy}
    # for i in range(-offset_distance,-offset_distance*(n_plies+1),-offset_distance):
    #     curves_offset[abs(i)] = np.array(offset_curve(curve_0,i).coords)

    # #%% mid index
    # chord_line = lineConstructor(p0 = sta.xy[0],
    #                             p1 = sta.xy[int(np.floor(len(sta.xy)/2))])

    # splines = {x:splineConstructor(curves_offset[x]) for x in range(n_plies+1)}
    # idx_LE = {x:splineIntersection(splines[x],chord_line,25) for
    #         x in range(n_plies+1)}
    ...
    # p_ref = np.array([splines[0]['x'](idx_LE[0]),splines[0]['y'](idx_LE[0])])
    # t_target = skinOverlapLocator(overlap_target,p_ref,sta.parameters['twist_angle'],
    #                             splines[0],idx_LE[0])
    ...
    # #Cut all other 8 lines at this point
    # for i in range(1,n_plies+1):
    #     idx_olp_sta[i] = splineIntersection(splines[i],overlap_line,idx_LE[i])
    ...
    # for i in range(n_plies+1):
    #     t = np.arange(int(np.ceil(idx_olp_sta[i]))).tolist() + [idx_olp_sta[i]]
    #     t_bot[i] = t
    ...
    # idx_TE_bot, idx_TE_top = trailingEdgeThickness(trailing_edge_thickness, splines[0])

    # te_line = lineConstructor(p0 = (splines[0]['x'](idx_TE_bot), splines[0]['y'](idx_TE_bot)),
    #                         p1 = (splines[0]['x'](idx_TE_top), splines[0]['y'](idx_TE_top)))

    # idx_te_bot = {x:splineIntersection(splines[x],te_line) 
    #             for x in range(n_plies+1)}

    # # %% Plot new graphs with te cut
    # fig, ax = plt.subplots(figsize=(10,6))
    ...
    # figs['bottom_trim'] = fig
    # if saveFig:
    #     fig.savefig('bottom_skin_2.png',dpi=800)

    # # %% Cut trailing edge to join splines to form ply
    ...
    #         te_ply_cut_line = lineConstructor(p0=(splines[i+1]['x'](idx_te_bot[i+1]),
    #                                             splines[i+1]['y'](idx_te_bot[i+1])),
    #                                         ang=te_line['tan'])
    #         idx_ply_cut_bot[i] = splineIntersection(splines[i],
    #                                                 te_ply_cut_line,
    #                                                 idx_te_bot[i])

    # print(idx_ply_cut_bot)
    ...
    #     if i not in idx_ply_cut_bot.keys():
    #         t_plies_bot[i+1][i] = t_splines_bot[i]
    #     else:
    #         t_plies_bot[i+1][i] = [idx_ply_cut_bot[i]] + [x for x in
    #                                 t_splines_bot[i] if x > idx_ply_cut_bot[i]]
            
    #     t_plies_bot[i+1][i+1] = t_splines_bot[i+1]
    ...
    # # Identify t for trimmed lines for plies
    ...
    #         te_ply_cut_line = lineConstructor(p0=(splines[i+1]['x'](idx_te_top[i+1]),
    #                                             splines[i+1]['y'](idx_te_top[i+1])),
    #                                         ang=te_line['tan'])
    #         idx_ply_cut_top[i] = splineIntersection(splines[i],
    #                                                 te_ply_cut_line,
    #                                                 idx_olp_sta[i])

    # print(idx_ply_cut_top)

    # for i in range(n_plies):
    #     t_plies_top[i+1] = {}
        
    #     if i not in idx_ply_cut_top.keys():
    #         t_plies_top[i+1][i] = t_splines_top[i]
    #     else:
    #         t_plies_top[i+1][i] = [x for x in t_splines_top[i] if 
    #                             x < idx_ply_cut_top[i]] + [idx_ply_cut_top[i]]
            
    #     t_plies_top[i+1][i+1] = t_splines_top[i+1]
